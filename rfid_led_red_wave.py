...
Same functionality as rfid_led.py, with ONE visual difference in the IDLE
state: the strip background stays solid WHITE and a RED "slide-to-centre"
wave runs constantly — two red fronts grow inward from both outer edges
until they meet in the middle, then the cycle repeats. The GREEN tag-blink is
unchanged.

Behaviour
---------
* Idle (no tags being scanned): the strip background is WHITE and a red wave
  continuously slides in from both edges toward the middle, looping forever.
* Every NEW unique tag reported by `rfid_reader` produces ONE visible green
  blink:
      ...
      - then back to the constant red slide-to-centre idle animation.
...
# Delay between each step of the edge-to-centre slide animation.
...
def idle_slide_until_tag(
    ...
    """Run the constant edge-to-centre `wave_color` slide over a `bg_color` strip.

    Each sweep starts from a fully `bg_color` strip and grows two `wave_color`
    fronts inward from the outer edges until they meet in the middle; then the
    sweep repeats, so the animation runs continuously. Between every frame we
    poll for a new tag event.
    ...
    while not stop_event.is_set():
        # Begin each sweep from a fully `bg_color` strip.
        for i in range(n):
        ...
        while left <= right:
            ...
            strip.setPixelColor(left, wave_color)
            strip.setPixelColor(right, wave_color)
            # ...and restore to the background the pixels that have fallen out
            # of the short tail behind each front, so only SLIDE_TAIL_PIXELS
            # stay coloured.
            tail_left = left - SLIDE_TAIL_PIXELS
            tail_right = right + SLIDE_TAIL_PIXELS
            if tail_left >= 0:
                strip.setPixelColor(tail_left, bg_color)
            if tail_right <= n - 1:
                strip.setPixelColor(tail_right, bg_color)
            strip.show()
...
def main() -> int:
    ...
    RED   = hex_to_color(RED_HEX)
    WHITE = hex_to_color(WHITE_HEX)
    OFF   = hex_to_color(OFF_HEX)

    # Default idle state: strip WHITE (the looping red slide takes over once
    ...
    def led_worker() -> None:
        """Render the constant red slide-to-centre idle / GREEN-blink
        behaviour described in the module docstring."""
        while not stop_event.is_set():
            # Idle: run the constant edge-to-centre RED slide over a WHITE
            # strip until a new tag arrives (or we are told to stop).
            got_tag = idle_slide_until_tag(
                strip, RED, WHITE, stop_event, tag_events
            )
            if not got_tag:
            ...
            # 3. Back to the constant red slide-to-centre idle animation
            #    (next loop iteration).
