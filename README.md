...
chmod +x compile.sh system.sh install_aux_led.sh
...
2. **Install / start `aux-led.service` if it isn't already** — so the GPIO13
   white LED comes on now and at every future boot. Idempotent on subsequent
   runs.
3. Check USB permissions for `/dev/ttyACM0` / `/dev/ttyUSB0`.
4. Compile `rfid_reader`.
5. Launch `rfid_led.py`, which spawns `rfid_reader` and watches its output.

> You can also install / uninstall the aux-LED service by hand at any time
> with `./install_aux_led.sh` and `./install_aux_led.sh uninstall`.
