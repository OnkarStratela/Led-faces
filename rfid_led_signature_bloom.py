...
"Signature glide + green bloom" — a copy of the signature-glide mode designed
for a matte-black bin chute, but with a re-imagined GREEN scan acknowledgment.

Rationale
---------
The bin is matte black, so the IDLE background is left OFF: against black, an
unlit strip reads as a seamless, intentional surface. On top of that void a
single CRISP FULL-WHITE light glides back and forth with a soft cool-white
gaussian halo and cosine easing, so it decelerates and "breathes" at each end
— a calm, premium standby light that draws the eye to the chute opening.

Unlike the other modes, the scan acknowledgment is NOT a flat blink. On every
new tag a bright green core ignites at the CENTRE of the strip and EXPANDS
outward to both edges with the same eased motion as the glide, holds at full
green for a beat, then DISSOLVES smoothly back to black before the white glide
resumes. It reads like the chute confirming and registering the deposit.

Two hard constraints are respected:
  1. Everything runs at full brightness. The WS2812 master brightness stays at
     255 and the GPIO13 PWM LED at 1.0; the halo, tail and bloom are shaped
     purely by per-pixel COLOUR, never by dimming the hardware. The glide head
     is pure white (255, 255, 255) and the bloom peaks at pure green.
  2. A scan is always GREEN. The acknowledgment colour is #00FF00; only its
     SHAPE/animation differs from the other modes.
...
def green_bloom_until_idle(
    ...
    Returns True when the bloom(s) finish cleanly (caller resumes the glide),
    ...
    while not stop_event.is_set():
        ...
        # Clean finish: strip fully OFF, then back to the idle glide.
        fill_strip(strip, 0)
        return True
...
def main() -> int:
    ...
    def led_worker() -> None:
        """Render the signature-glide idle / GREEN-bloom behaviour described in
