...
state: the strip background stays solid WHITE and a GREEN "slide-to-centre"
wave runs constantly — two green fronts grow inward from both outer edges
until they meet in the middle, then the cycle repeats. The GREEN tag-blink is
unchanged.

Behaviour
---------
* Idle (no tags being scanned): the strip background is WHITE and a green wave
...
* Every NEW unique tag reported by `rfid_reader` produces ONE visible green
  blink:
      ...
      - then back to the constant green slide-to-centre idle animation.
...
def main() -> int:
    ...
    # Default idle state: strip WHITE (the looping green slide takes over once
    ...
    def led_worker() -> None:
        """Render the constant green slide-to-centre idle / GREEN-blink
        behaviour described in the module docstring."""
        while not stop_event.is_set():
            # Idle: run the constant edge-to-centre GREEN slide over a WHITE
            # strip until a new tag arrives (or we are told to stop).
            got_tag = idle_slide_until_tag(
                strip, GREEN, WHITE, stop_event, tag_events
            )
            if not got_tag:
            ...
            # 3. Back to the constant green slide-to-centre idle animation
            #    (next loop iteration).
